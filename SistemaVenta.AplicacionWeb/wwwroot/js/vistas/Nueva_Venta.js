...
let tipoPago = cboTipoPago.value;
...
function mostrarModalPago(modelo = MODELO_BASE_PAGO) {
    ...
    cboTipoPago.addEventListener('change', function () {
        ...
        if (this.value === '0') {
            // Cambiar el valor por defecto a 1
            this.value = '1';
        }
    });

    tipoPago = cboTipoPago.value
...
function registrarVenta() {
    ...
    const pago = structuredClone(MODELO_BASE_PAGO);
        pago["idPago"] = 0
        pago["monto"] = $("#txtMonto").val()
